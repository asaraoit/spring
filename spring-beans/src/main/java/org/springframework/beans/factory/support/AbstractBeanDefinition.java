...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/*
	 * bean 的 className 对应 bean 属性 class
	 */
	@Nullable
	...
	/*
	 * 用来表示一个 bean 的实例化依靠另一个 bean 先实例化 depend-on
	 */
	@Nullable
	 ...
	 *  将不考虑该 bean，即它不会被考虑作为其他 bean 自动装配的候选者，但是该 bean本身还是可以使用自动装配
	 *  来注入其他 bean 的
	 *  对应bean 属性 autowire-candidate
	 */
	 ...
	 * <bean id="instanceFactoryBean" class="com.*.beans.InstanceFactoryBean"/>
	...
	/*
	 * 方法重写的持有者 记录 lookup-method,replaces-method 元素
	 */
	...
	/*
	 * 定义这个bean 的应用，APPLICATION：用户，INFRASTRUCTRURE：完全内部使用，与用户为无管
	 * SUPPORT：某些复杂配置的一部分
	 * 程序设置
	 */
